...
(function() {
    ...
    mango.filters = {
        ...
        date: function(str, arg){
            ...
            str = new Date(str);
            str = new Date(str.getTime() + (str.getTimezoneOffset() * 60 * 1000));
            return mango.strftime(str, arg);
        },
        ...
        removetags: function(str, arg) {
            var tags = '(' + arg.split(' ').join('|') + ')',
                starttag_re = new RegExp('<' + tags + '(/?>|(\s+[^>]*>))', 'gi'),
                endtag_re = new RegExp('</' + tags + '>', 'gi');
            str = str.replace(starttag_re, '');
            str = str.replace(endtag_re, '');
            return str;
        },
    ...
    mango.tags = {
        ...
        "for": function(args){
            ...
            for (i=0; i < loopArgs.length; i ++) {
                // What is the purpose of this code block?  If the first item in the list is an Object, why treat it differently?
